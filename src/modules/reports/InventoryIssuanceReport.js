...
const InventoryIssuanceReport = (props) => {
  ...
  const issuanceGridColumms = [
    "Photo", "Name", "Designation", "Emp Number", "Prison", "BPS", "Item Type", "Item",
    "Qty Demanded", "Qty Issued", "Issued from Batch", "Serial Number"
  ]
  const headers = issuanceGridColumms.slice(1)
  ...
  return (
    <>
      <div className="row">
        <h3 className="third-heading db-heading">Issuance Wise Report</h3>
      </div>
      ...
      <div className="row p-2">
        <form onSubmit={handleGenerateReport}>
          <div className="row">
            <div className="row">
              <InventorySearch type={"issuance"} formPayload={formPayload} setFormPayload={setFormPayload} />
            </div>
          </div>
      ...
      <div className="card custom-card animation-fade-grids custom-card-scroll mt-4">
        <TableGrid thArr={issuanceGridColumms} tdArr={TreatmentData} />
      </div>
    </>
  );
};

export default InventoryIssuanceReport;
