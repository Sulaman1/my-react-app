...
import PriscriptionModal from '../Prescribe/PriscriptionModal';
import PriscriptionGrid from '../../../hospital/components/PriscriptionGrid';
import { postData } from '../../../../services/request';

const PrescribeInformation = (props) => {
  ...
  const showPriscriptionsHandler = async () => {
    ...
    try {
      const res = await postData(
        `/services/app/Medicine/GetIssuableMedicineForEmployee`,
        payload
      );
  ...
  return (
    <>
      ...
      <div className="mt-4 mb-4 d-flex justify-content-center gap-2">
        ...
        >
          <i className="icon-add ml-2"></i> Dispense Medicine
        </button>
      </div>
    </>
  );
...
export default PrescribeInformation;
